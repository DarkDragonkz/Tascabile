...
export class MangaWorld
    ...
    async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        const trendingItems = this.parser.parseHomeSectionItems($, '#chapters-slide .entry.vertical')
        const latestItems = this.parser.parseHomeSectionItems($, '.comics-grid .entry')
    ...
    async getViewMoreItems(homepageSectionId: string, metadata: unknown): Promise<PagedResults> {
        ...
        const sort = homepageSectionId === 'trending'
            ? 'most_read'
            : 'newest'
