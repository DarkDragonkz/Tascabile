...
export class BatcaveParser {
  ...
  parseListItems($: CheerioAPI): BatcaveListItem[] {
    const items: BatcaveListItem[] = []

    $("#dle-content > .readed.short").each((_, el) => {
      const root = $(el)
      ...
      const description = infoLis.find((x) => !/^Last issue:/i.test(x))
      const ratingPercent = this.parseRatingPercent(root.find(".current-rating").first().attr("style"))
      const votes = this.parseInteger(root.find(".readed__rating-votes span").first().text())
      const readUrl = this.attrAbsolute(root.find(".readed__btn[href]").first(), "href")

      const item: BatcaveListItem = {
        title,
        url: this.absoluteUrl(href),
      }

      if (imageRaw && !imageRaw.startsWith("data:")) item.image = this.absoluteUrl(imageRaw)
      if (meta[0]) item.publisher = meta[0]
      if (meta[1]) item.year = meta[1]
      if (description) item.description = description
      if (lastIssueRaw) item.lastIssue = lastIssueRaw.replace(/^Last issue:\s*/i, "").trim()
      if (ratingPercent !== undefined) item.ratingPercent = ratingPercent
      if (votes !== undefined) item.votes = votes
      if (readUrl) item.readUrl = readUrl
